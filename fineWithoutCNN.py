...
if __name__ == '__main__':
    ...
    if(os.path.exists('./data/' + data_directory + '/model/' + bottom_model_name + '_withoutCNN.h5')):
        model.load_weights(os.path.join('./data/' + data_directory + '/model/' + bottom_model_name + '_withoutCNN.h5'))

    model.compile(loss='categorical_crossentropy',
                  optimizer=optimizers.SGD(lr=1e-4, momentum=0.9),
                  metrics=['accuracy'])
    checkpoint = ModelCheckpoint('./data/' + data_directory + '/model/' + bottom_model_name + '_withoutCNN.h5', monitor='val_acc', verbose=1, save_best_only=False)
    model.fit_generator(
        train_data,
